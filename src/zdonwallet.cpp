...
CzDONWallet::CzDONWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdon seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDONSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDONSeed_deprecated()) {
                    LogPrintf("%s: Updated zDON seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdon seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdon wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDONWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDONCount(nCountLastUsed);
    else if (!walletdb.ReadZDONCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDONWallet::Lock()
{
    seedMaster = 0;
}

void CzDONWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDONWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDON(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDONWallet::LoadMintPoolFromDB()
{
...
void CzDONWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDONWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDONWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdonTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDONWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDON(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdonTracker which also adds to database
    pwalletMain->zdonTracker->Add(dMint, true);
...
void CzDONWallet::SeedToZDON(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDONWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDONWallet::GenerateDeterministicZDON(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDONWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDONWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
