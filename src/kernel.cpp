...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZDonStake(spend));
    } else {
        ...
        CDonStake* donInput = new CDonStake();
        donInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(donInput);
    }
