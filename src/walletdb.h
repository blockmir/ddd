...
public:
    ...
    bool WriteZDONSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDONSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDONSeed_deprecated(uint256& seed);
    bool EraseZDONSeed();
    bool EraseZDONSeed_deprecated();

    bool WriteZDONCount(const uint32_t& nCount);
    bool ReadZDONCount(uint32_t& nCount);
