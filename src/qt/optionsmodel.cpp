...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::DON);
    ...
    if (!settings.contains("nAnonymizeDonateAmount"))
        settings.setValue("nAnonymizeDonateAmount", 1000);

    nAnonymizeDonateAmount = settings.value("nAnonymizeDonateAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeDonateAmount"))
        SoftSetArg("-anonymizedonateamount", settings.value("nAnonymizeDonateAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in donate.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeDonateAmount:
            return QVariant(nAnonymizeDonateAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeDonateAmount:
            nAnonymizeDonateAmount = value.toInt();
            settings.setValue("nAnonymizeDonateAmount", nAnonymizeDonateAmount);
            emit anonymizeDonateAmountChanged(nAnonymizeDonateAmount);
            break;
        case CoinControlFeatures:
