...
#include "ui_zdoncontroldialog.h"
...
std::set<std::string> ZDonControlDialog::setSelectedMints;
std::set<CMintMeta> ZDonControlDialog::setMints;

bool CZDonControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZDonControlDialog::COLUMN_DENOMINATION || column == ZDonControlDialog::COLUMN_VERSION || column == ZDonControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZDonControlDialog::ZDonControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDonControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDonControlDialog::~ZDonControlDialog()
{
    delete ui;
}

void ZDonControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDonControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZDonControlWidgetItem* itemDenom(new CZDonControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZDonControlWidgetItem *itemMint = new CZDonControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
...
void ZDonControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDonControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDon_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDonControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDonControlDialog::GetSelectedMints()
{
...
void ZDonControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
