...
#ifndef DONATE_ZDONWALLET_H
#define DONATE_ZDONWALLET_H
...
public:
    CzDONWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDON(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDON(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DONATE_ZDONWALLET_H
